...
def dynamic(t, y, m, im, g, f, wh, kw):
    ...
    dR = np.dot(R, skw(w))
    dw = np.dot(np.power(im, -1, where=(im!=0)), (np.dot(skw(np.dot(im, w)), w) + tau))
...
class DroneLQR:
    def __init__(self, optimal_distance=0.0):
        ...
        if self.ts == 0.01:
            ...
            self.Df = 0
            self.xf1 = np.array([[0], [0], [self.rp[0][0]]]) ######## changed this row 
            self.xf2 = np.array([[0], [0], [self.rp[1][0]]]) ######## changed this row 
            self.xf3 = np.array([[0], [0], [self.rp[2][0]]]) ######## changed this row 
        else:
    ...
    def perform_action(self, observation, take_off=False) -> tuple:
        ...
        rf = np.vstack([rf1[0], rf2[0], rf3[0]]) ######## changed this row 
        # print("rf: {}".format(rf))
        # print("p0: {}".format(p0))
        inpcop = rf - p0
        # print("inpcop: {}".format(inpcop))
        u1 = np.dot(self.kdC, self.xest1) + np.dot(self.kdD, inpcop[0][0]) ######## changed this row 
        self.xest1 = np.dot(self.KdA, self.xest1) + np.dot(self.KdB, inpcop[0][0]) ######## changed this row 
        u2 = np.dot(self.kdC, self.xest2) + np.dot(self.kdD, inpcop[1][0]) ######## changed this row 
        self.xest2 = np.dot(self.KdA, self.xest2) + np.dot(self.KdB, inpcop[1][0]) ######## changed this row 
        u3 = np.dot(self.kdC, self.xest3) + np.dot(self.kdD, inpcop[2][0]) ######## changed this row 
        self.xest3 = np.dot(self.KdA, self.xest3) + np.dot(self.KdB, inpcop[2][0]) ######## changed this row 
        u = - np.array([u1, u2, u3]) ######## changed this row 

        # Output
        omega3 = yaw_control(R, yaw, self.kth)
        f = np.exp(self.lk) * self.mnom
        q = np.dot(R.T, u) / np.exp(self.lk) ######## changed this row 
        print("q: {}".format(q))
        wtr = np.zeros((3, 1))
        wtr[0] = -q[1] ######## changed this row 
        wtr[1] = q[0] ######## changed this row 
        wtr[2] = omega3 ######## changed this row 
        # wtr[2] = 0.0
        dlam = q[2]
